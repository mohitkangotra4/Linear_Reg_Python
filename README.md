...
## Result
Using coefficients derived from multiple linear regression to see how much return we can derive from each dollar we spend. We observed clearly that for different account type(facilities i.e., private, small, medium, large) different Campaigns and Different Sales Contact are effective with different extend.
